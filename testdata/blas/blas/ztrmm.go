...
//*>
//*  =====================================================================
func ZTRMM(SIDE *byte, UPLO *byte, TRANSA *byte, DIAG *byte, M *int, N *int, ALPHA *complex128, A *[][]complex128, LDA *int, B *[][]complex128, LDB *int) {
	var TEMP complex128
	var I int
	var INFO int
	var J int
	...
	if INFO != 0 {
		XERBLA(func() *[]byte { y := []byte("ZTRMM "); return &y }(), &(INFO))
		return
	}
	//*
				...
				for K = 1; K <= (*N); K++ {
					for J = 1; J <= K-1; J++ {
						if (*A)[J-(1)][K-(1)] != ZERO {
							if NOCONJ {
								...
								TEMP = (*ALPHA) * intrinsic.DCONJG((*A)[J-(1)][K-(1)])
							}
							for I = 1; I <= (*M); I++ {
								(*B)[I-(1)][J-(1)] = (*B)[I-(1)][J-(1)] + TEMP*(*B)[I-(1)][K-(1)]
							}
					...
					if NOUNIT {
						if NOCONJ {
							...
							TEMP = TEMP * intrinsic.DCONJG((*A)[K-(1)][K-(1)])
						}
					}
					if TEMP != ONE {
						for I = 1; I <= (*M); I++ {
