...
//*>
//*  =====================================================================
func ZHERK(UPLO *byte, TRANS *byte, N *int, K *int, ALPHA *float64, A *[][]complex128, LDA *int, BETA *float64, C *[][]complex128, LDC *int) {
	var TEMP complex128
	var RTEMP float64
	var I int
	var INFO int
		...
		if UPPER {
			for J = 1; J <= (*N); J++ {
				for I = 1; I <= J-1; I++ {
					TEMP = ZERO
					for L = 1; L <= (*K); L++ {
						TEMP = TEMP + DCONJG(&((*A)[L-(1)][I-(1)]))*(*A)[L-(1)][J-(1)]
					}
					if (*BETA) == ZERO {
				...
				RTEMP = ZERO
				for L = 1; L <= (*K); L++ {
					RTEMP = RTEMP + DCONJG(&((*A)[L-(1)][J-(1)]))*(*A)[L-(1)][J-(1)]
				}
				if (*BETA) == ZERO {
