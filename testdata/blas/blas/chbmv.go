...
//*>
//*  =====================================================================
func CHBMV(UPLO *byte, N *int, K *int, ALPHA *complex64, A *[][]complex64, LDA *int, X *[]complex64, INCX *int, BETA *complex64, Y *[]complex64, INCY *int) {
	...
	if LSAME(UPLO, func()*byte{y:=byte('U');return &y}()) {
		...
		if ((*INCX) == 1) && ((*INCY) == 1) {
			for J = 1; J <= (*N); J++ {
				TEMP1 = (*ALPHA) * (*X)[J-(1)]
				TEMP2 = ZERO
				(*Y)[J-(1)] = (*Y)[J-(1)] + TEMP1*real((*A)[1-(1)][J-(1)])
				L = 1 - J
			...
			for J = 1; J <= (*N); J++ {
				TEMP1 = (*ALPHA) * (*X)[JX-(1)]
				TEMP2 = ZERO
				(*Y)[JY-(1)] = (*Y)[JY-(1)] + TEMP1*real((*A)[1-(1)][J-(1)])
				L = 1 - J
				IX = JX
				IY = JY
				for I = J + 1; I <= intrinsic.MIN((*N), J+(*K)); I++ {
