...
//*>
//*  =====================================================================
func CTRSM(SIDE *byte, UPLO *byte, TRANSA *byte, DIAG *byte, M *int, N *int, ALPHA *complex64, A *[][]complex64, LDA *int, B *[][]complex64, LDB *int) {
	var TEMP complex64
	var I int
	var INFO int
	var J int
	...
	if INFO != 0 {
		XERBLA(func()*[]byte{y:=[]byte("CTRSM ");return &y}(), &(INFO))
		return
	}
	//*
				...
				for K = (*N); K <= 1; K += -1 {
					if NOUNIT {
						if NOCONJ {
							...
							TEMP = ONE / intrinsic.CONJG((*A)[K-(1)][K-(1)])
						}
						for I = 1; I <= (*M); I++ {
					...
					for J = 1; J <= K-1; J++ {
						if (*A)[J-(1)][K-(1)] != ZERO {
							if NOCONJ {
								...
								TEMP = intrinsic.CONJG((*A)[J-(1)][K-(1)])
							}
							for I = 1; I <= (*M); I++ {
