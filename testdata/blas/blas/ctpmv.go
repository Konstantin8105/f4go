...
//*>
//*  =====================================================================
func CTPMV(UPLO *byte, TRANS *byte, DIAG *byte, N *int, AP *[]complex64, X *[]complex64, INCX *int) {
	...
	if INFO != 0 {
		XERBLA(func()*[]byte{y:=[]byte("CTPMV ");return &y}(), &(INFO))
		return
	}
	//*
			...
			if (*INCX) == 1 {
				for J = (*N); J <= 1; J += -1 {
					...
					if NOCONJ {
						...
						if NOUNIT {
							TEMP = TEMP * CONJG(&((*AP)[KK-(1)]))
						}
						for I = J - 1; I <= 1; I += -1 {
							TEMP = TEMP + CONJG(&((*AP)[K-(1)]))*(*X)[I-(1)]
							K = K - 1
						}
					}
					(*X)[J-(1)] = TEMP
