...
//*>
//*  =====================================================================
func ZTPSV(UPLO *byte, TRANS *byte, DIAG *byte, N *int, AP *[]complex128, X *[]complex128, INCX *int) {
	...
	if INFO != 0 {
		XERBLA(func()*[]byte{y:=[]byte("ZTPSV ");return &y}(), &(INFO))
		return
	}
	//*
			...
			if (*INCX) == 1 {
				for J = 1; J <= (*N); J++ {
					...
					if NOCONJ {
						...
						for I = 1; I <= J-1; I++ {
							TEMP = TEMP - intrinsic.DCONJG((*AP)[K-(1)])*(*X)[I-(1)]
							K = K + 1
						}
						if NOUNIT {
							TEMP = TEMP / intrinsic.DCONJG((*AP)[KK+J-1-(1)])
						}
					}
					(*X)[J-(1)] = TEMP
					KK = KK + J
				...
				for J = 1; J <= (*N); J++ {
					...
					if NOCONJ {
						...
						for K = KK; K <= KK+J-2; K++ {
							TEMP = TEMP - intrinsic.DCONJG((*AP)[K-(1)])*(*X)[IX-(1)]
							IX = IX + (*INCX)
						}
						if NOUNIT {
							TEMP = TEMP / intrinsic.DCONJG((*AP)[KK+J-1-(1)])
			...
			if (*INCX) == 1 {
				for J = (*N); J <= 1; J += -1 {
					...
					if NOCONJ {
						...
						if NOUNIT {
							TEMP = TEMP / intrinsic.DCONJG((*AP)[KK-(*N)+J-(1)])
						}
					}
					(*X)[J-(1)] = TEMP
					KK = KK - ((*N) - J + 1)
