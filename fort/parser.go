...
func (p *parser) init(fset *FileSet, filename string, src []byte, mode ModeScanner) {
...
func syncStmt(p *parser) {
	for {
		switch p.tok {
		case BREAK, CONST, CONTINUE, DEFER,
			FALLTHROUGH, FOR, GO, GOTO,
			IF, RETURN, SELECT, SWITCH,
			TYPE, VAR:
...
func (p *parser) parseMethodSpec(scope *ast.Scope) *ast.Field {
	...
	var typ ast.Expr
	x := p.parseTypeName()
	if ident, isIdent := x.(*ast.Ident); isIdent && p.tok == LPAREN {
		// method
		idents = []*ast.Ident{ident}
		scope := ast.NewScope(nil) // method scope
		params, results := p.parseSignature(scope)
		typ = &ast.FuncType{Func: NoPos, Params: params, Results: results}
	} else {
		// embedded interface
		typ = x
		p.resolve(typ)
...
func (p *parser) parseUnaryExpr(lhs bool) ast.Expr {
	...
	case ARROW:
		...
		// <-(expr)
		return &ast.UnaryExpr{OpPos: arrow, Op: ARROW, X: p.checkExpr(x)}
...
func (p *parser) parseSimpleStmt(mode int) (ast.Stmt, bool) {
	...
	case
		...
		if mode == rangeOk && p.tok == RANGE && (tok == DEFINE || tok == ASSIGN) {
			pos := p.pos
			p.next()
			y = []ast.Expr{&ast.UnaryExpr{OpPos: pos, Op: RANGE, X: p.parseRhs()}}
			isRange = true
		} else {
			y = p.parseRhsList()
		}
...
func (p *parser) parseBranchStmt(tok Token) *ast.BranchStmt {
	if p.trace {
...
type parseSpecFunction func(doc *ast.CommentGroup, keyword Token, iota int) ast.Spec
...
func (p *parser) parseImportSpec(doc *ast.CommentGroup, _ Token, _ int) ast.Spec {
	if p.trace {
	...
	spec := &ast.ImportSpec{
		Doc:     doc,
		Name:    ident,
		Path:    &ast.BasicLit{ValuePos: pos, Kind: STRING, Value: path},
...
func (p *parser) parseValueSpec(doc *ast.CommentGroup, keyword Token, iota int) ast.Spec {
	if p.trace {
...
func (p *parser) parseTypeSpec(doc *ast.CommentGroup, _ Token, _ int) ast.Spec {
	if p.trace {
...
func (p *parser) parseGenDecl(keyword Token, f parseSpecFunction) *ast.GenDecl {
	if p.trace {
